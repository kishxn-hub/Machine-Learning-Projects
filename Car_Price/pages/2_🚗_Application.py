...
#page name and icon
st.set_page_config(page_title="CarPrice-Application",page_icon='car.jpg')
...
#title using css and center align
st.markdown("<h1 style='text-align: center; color: cyan;'>CAR PRICE PREDICTOR APPLICATION</br>------------------------------------------------</h1>", unsafe_allow_html=True)

#importing image
from PIL import Image
imagg = Image.open('car.jpg')
...
#creating button with name
if st.button('Predict Price'):
    #storing inputs to corresponding columns
    input_df = pd.DataFrame({'name': [name], 'company': [company], 'year': [year],
                             'fuel_type': [fuel_type], 'kms_driven': [kms_driven]})
    #passing inputs to model as parameter using predict func
    pp=pipe.predict(input_df)
    pp = int(pp)
    st.header("Price of "+str(name)+" is Rs "+str(pp)+" !! ")
